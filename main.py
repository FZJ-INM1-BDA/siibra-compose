...
def get_module_path(arg, git_url):
    if Path(arg).is_dir():
        ...
        module_path = mkdtemp()
        subprocess.run(["git", "clone", "-b", arg, git_url, module_path])
        used_tmp = True
    def callback():
        if not used_tmp:
            return
...
def main(config, spy, sapi, sxplr, clean):
    ...
    config_path, config_callback = get_module_path(config, "https://jugit.fz-juelich.de/t.dickscheid/brainscapes-configurations.git")
    cleanup_callbacks.append(config_callback)
    
    spy_path, spy_callback = get_module_path(spy, "https://github.com/FZJ-INM1-BDA/siibra-python.git")
    cleanup_callbacks.append(spy_callback)

    sapi_path, sapi_callback = get_module_path(sapi, "https://github.com/fzj-inm1-bda/siibra-api.git")
